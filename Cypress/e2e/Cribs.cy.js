...
describe("Cribs Collection - Best Sellers with variants", () => {
  ...
  it("When the first product has more than one variant, should open PDP, change preselected option, and assert images update", () => {
    ProductsListCribsBestSellers.getFirstProductSwatchCount().then((count) => {
      ...
      // Open product: click first best-sellers product link
      ProductsListCribsBestSellers.openFirstProductPDP();

      // Assert we are on a PDP (product URL)
      cy.url().should("include", "/products/");

      // On PDP: use variant list in sidebar, pick a different option than default,
      // and assert product images change (command in support/commands.js)
      ...
      const mainImageSelector =
        '.meet-the-product__image-container img, .product-main img[src*="cdn.shopify"], #MainContent img[src*="cdn.shopify"], [id*="main-product"] img[src*="cdn.shopify"], .product-main img';
      cy.get("body").then(($body) => {
        ...
        cy.get(mainImageSelector, { timeout: 20000 })
          .first()
          .invoke("attr", "src")
          .then((srcBefore) => {
